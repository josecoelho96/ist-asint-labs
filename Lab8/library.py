...
class library:
    ...
    def listBooks(self, name=''):
        ...
        if name == '':
            for b in self.bib.values():
                ret_value.append({'id': b.id, 'author': b.author,
                                  'title': b.title, 'date': b.year, 'votes': b.sum_votes})
        else:
            for b in self.bib.values():
                if b.author == name:
                    ret_value.append(
                        {'id': b.id, 'author': b.author, 'title': b.title, 'date': b.year, 'votes': b.sum_votes})
